...
public class MailGenerator {
     ...
     * @param wModem              the WifiModem that can be access
     * @param activity_unit_price the activity_unit_price
     * @param markup_percentage   the markup_percentage
     */
    public MailGenerator(int mailToCreate, int mailMaxWeight, MailPool mailPool, HashMap<Boolean, Integer> seed,
            WifiModem wModem, double activity_unit_price, double markup_percentage) {
    ...
    private MailItem generateMail() {
        ...
        double service_cost = generateServiceCost(destinationFloor);

        newMailItem = new MailItem(destinationFloor, arrivalTime, weight, service_cost, activity_unit_price,
                markup_percentage);
        return newMailItem;
    }

    /**
     * @return Service Cost according to the destination floor
     */
    private double generateServiceCost(int destinationFloor) {
        return wModem.forwardCallToAPI_LookupPrice(destinationFloor);
    }
    ...
    /**
     * This class initializes all mails and sets their corresponding values, All
     * generated mails will be saved in allMail
     */
    ...
    /**
     * Given the clock time, put the generated mails into the mailPool. So that the
     * robot will can pick up the mails from the pool.
     */
